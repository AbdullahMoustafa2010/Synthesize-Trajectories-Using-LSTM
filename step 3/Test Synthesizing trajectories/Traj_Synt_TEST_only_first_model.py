...
#           Model=Model_Quarter9
        ...
        if UNNormalized_Created_ut_y >= 480 or UNNormalized_Created_ut_x >= 360:
            break
        #--------------------------------------------------------------------------
...
#Ploting in Python
import matplotlib.pyplot as plt
im = plt.imread('000000_Q2.jpg')
implot = plt.imshow(im)
#put a blue dot at (10, 20)
for k in range (0,len(list_all)-1):
    u=list_all[k]
    plt.scatter([row[0] for row in u],[row[1] for row in u],s=1)
#    plt.scatter([row[0] for row in u],[row[1] for row in u],linewidth=1)
